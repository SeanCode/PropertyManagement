...
public interface DepartmentDao extends JpaRepository<DepartmentEntity, Long> {
    ...
    @Query(value = "select * from department where id = :id and weight >= 0", nativeQuery = true)
    DepartmentEntity getDepartmentById(@Param("id") Long id);

    @Query(value = "select * from department where name = :name and weight >= 0", nativeQuery = true)
    DepartmentEntity getDepartmentByName(@Param("name") String name);

    @Query(value = "select * from department where parent_id = :parent_id and name = :name and weight >= 0", nativeQuery = true)
    DepartmentEntity getDepartmentByParentIdAndName(@Param("parent_id") Long parentId, @Param("name") String name);

    @Query(value = "select * from department where root_id = :root_id and weight >= 0", nativeQuery = true)
